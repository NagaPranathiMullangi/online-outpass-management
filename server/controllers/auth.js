...
export const studentVerification = async (req, res) => {
  ...
  try {
    ...
    if (existingUser) {
      return res
        .status(404)
        .json({ message: "User exists with same enrollment number" });
    }
  ...
  try {
    ...
    if (data) {
      ...
      const token = jwt.sign(
        { enrollment: newUser.enrollment, id: newUser._id },
        "test",
        { expiresIn: "1h" }
      );
...
export const studentLogin = async (req, res) => {
  ...
  try {
    ...
    const token = jwt.sign(
      { enrollment: existingUser.enrollment, id: existingUser._id },
      "test",
      { expiresIn: "1h" }
    );
...
export const wardenVerification = async (req, res) => {
  ...
  try {
    ...
    if (existingUser) {
      return res
        .status(404)
        .json({ message: "User exists with same employee number" });
    }
    const generatedOTP = Math.floor(1000 + Math.random() * 9000);
    await emailOtp.create({ email: email, otp: generatedOTP });
    mailer(email, generatedOTP);
...
export const wardenSignup = async (req, res) => {
  ...
  try {
    ...
    if (data) {
      const hashedPassword = await bcrypt.hash(password, 12);
      const newUser = await warden.create({
        name,
        employee,
        email,
        password: hashedPassword,
      ...
      const token = jwt.sign(
        { employee: newUser.employee, id: newUser._id },
        "test",
        { expiresIn: "1h" }
      );
      res.status(200).json({ result: newUser, token });
...
export const wardenLogin = async (req, res) => {
  ...
  try {
    ...
    const token = jwt.sign(
      { employee: existingUser.employee, id: existingUser._id },
      "test",
      { expiresIn: "1h" }
    );
    res.status(200).json({ result: existingUser, token });
